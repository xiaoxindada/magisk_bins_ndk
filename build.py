...
def build_rust_src(targets: set):
    ...
    os.chdir(Path(paths().project_root))
...
def clean_elf():
    ...
    cargo_toml = Path(paths().project_root, "tools", "elf-cleaner", "Cargo.toml")
...
def setup_ndk():
    ...
    ondk_path = Path(paths().project_root, f"ondk-{ondk_version}")

    if not op.exists(ndk_archive) and not op.exists(paths().ndk) or op.exists(paths().ndk) and open(paths().ndk / "ONDK_VERSION").read().strip(" \t\r\n") != ondk_version:
...
def update_code():
    ...
    magisk_version = cmd_out("cd Magisk && git rev-parse --short=8 HEAD && cd ..").strip(" \t\r\n")
    ondk_version = None
