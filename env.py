...
print(f"ondk_version: {ondk_version}")
...
class Paths:
    def __init__(self):
        ...
        self.ndk = self.project_root / "ndk"
        self.ndk_build = self.ndk / "ndk-build"
        self.rust_sysroot = self.ndk / "toolchains" / "rust"
        self.native = self.project_root / "native"
        self.native_out = self.native / "out"
        self.rust_bin = self.rust_sysroot / "bin"
        self.llvm_bin = (
            self.ndk / "toolchains" / "llvm" / "prebuilt" / f"{os_name}-x86_64" / "bin"
        )
        self.native_gen = self.native_out / "generated"
        self.rust_out = self.native_out / "rust"
        self.cargo = self.ndk / "toolchains" / "rust" / "bin" / f"cargo{EXE_EXT}"
...
def ensure_cargo():
    ...
    if shutil.which("rustup"):
        ...
        os.environ["PATH"] = (
            paths().rust_sysroot / "bin" / os.pathsep / os.environ["PATH"]
        )
